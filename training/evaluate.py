...
def load_corruption_data(data_dir: str, corruption: str, severity: int,
                          normalize, batch_size: int = 256):
    """
    ...
    images = np.load(images_path)  # shape: (50000*5, 32, 32, 3) — all severities stacked
...
def evaluate_clean(model, dataset: str, data_dir: str, normalize, device, batch_size=256):
...
def run_corruption_evaluation(model, data_dir: str, dataset: str, normalize, device, batch_size=256):
    """
    ...
    for corruption in CORRUPTIONS:
        ...
        for severity in SEVERITIES:
            loader = load_corruption_data(data_dir, corruption, severity, normalize, batch_size)
            if loader is None:
                continue
...
def print_results_table(results: dict, clean_acc: float, mce: float):
    ...
    console.print(f"[bold]mCE (↓ better):[/bold]  {mce*100:.2f}%" if mce else "mCE: N/A")

def save_results(results: dict, clean_acc: float, mce: float, checkpoint_path: str, output_dir: str = "results"):
    ...
    df.loc[len(df)] = {"corruption": "SUMMARY",
                        "mean_error": 1 - clean_acc}  # clean error
    ...
    ax.axhline(y=(1 - clean_acc) * 100, color="green",  linestyle="--", label=f"Clean error ({(1-clean_acc)*100:.1f}%)")
    ax.axhline(y=mce * 100 if mce else 0, color="red", linestyle="--", label=f"mCE ({mce*100:.1f}%)" if mce else "mCE")
...
def main():
    ...
    parser.add_argument("--data_dir",      type=str, required=True, help="Path to CIFAR-10-C or CIFAR-100-C folder")
    ...
    clean_acc = evaluate_clean(model, args.dataset, args.data_dir, normalize, device, args.batch_size)
    console.print(f"Clean Accuracy: [green]{clean_acc*100:.2f}%[/green]")

    # Corruption robustness
    results = run_corruption_evaluation(model, args.data_dir, args.dataset, normalize, device, args.batch_size)
