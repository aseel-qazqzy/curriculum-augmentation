...
def get_cifar10_loaders(
    ...
    print(
        f"CIFAR-10 loaded | Train: {len(train_dataset):,} | Val: {val_str} | Test: {len(test_dataset):,}"
    )
...
def get_tiny_imagenet_loaders(
    ...
    if val_split == 0.0:
        ...
        indices = torch.randperm(
            len(full_train), generator=torch.Generator().manual_seed(42)
        )
        train_ds = torch.utils.data.Subset(
            full_train, indices[: len(full_train) - val_size]
        )
        val_ds = torch.utils.data.Subset(
            full_val, indices[len(full_train) - val_size :]
        )
    ...
    print(
        f"Tiny-ImageNet loaded | Train: {len(train_ds):,} | "
