...
ABLATION_GROUPS["all"] = [
    cfg for group_cfgs in ABLATION_GROUPS.values() for cfg in group_cfgs
]
...
def run_ablation_group(group: str, debug: bool = False):
    if group not in ABLATION_GROUPS:
        raise ValueError(
            f"Unknown group '{group}'. Available: {list(ABLATION_GROUPS.keys())}"
        )
    ...
    for i, cfg in enumerate(configs):
        ...
        print(
            f"  cl_schedule={cfg.get('cl_schedule')}  "
...
def print_ablation_summary(group: str, results: list):
    ...
    for r in results:
        ...
        print(
            f"  {r['name']:<50} {r['best_val']:>8.2f}%  {r['test_top1']:>8.2f}%{marker}"
        )
    ...
    if len(results) >= 2:
        sorted_r = sorted(
            [r for r in results if r["best_val"]],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--debug", action="store_true", help="Quick 2-epoch test run")
