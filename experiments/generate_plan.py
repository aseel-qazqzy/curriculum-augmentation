...
EXPERIMENTS = [
    ...
    (
        ...
        42,
        100,
        "Tiny ImageNet lower bound",
    ),
    (
        "D2",
        "D",
        "Tiny ImageNet — Static",
...
def build_legend(wb: openpyxl.Workbook):
    ...
    group_desc = [
        (
            "B",
            "Internal baselines — no aug / static / random / RandAugment / static+mixing",
        ),
        (
            "B_COMBO",
            "Manual pipeline baselines — standard combinations used in CIFAR papers",
        ),
        (
            "B_PAPER",
            "Published method baselines — AutoAugment, TrivialAugment, AugMix, etc.",
        ),
        ("M", "Main CL runs — ETS vs LPS × mix_mode  (core thesis comparison)"),
        ("A_ETS", "Ablation — ETS tier-boundary fractions (tier_t1, tier_t2)"),
        (
            "A_LPS",
            "Ablation — LPS hyperparameters (tau / window / min_epochs_per_tier)",
        ),
