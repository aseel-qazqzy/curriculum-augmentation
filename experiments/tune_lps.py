...
def main():
    ...
    parser.add_argument("--milestones",      type=int,   nargs="+", default=[33, 66, 83],
                        help="MultiStepLR milestone epochs for grace-period simulation")
    parser.add_argument("--ets_t3",          type=int,   default=46,
                        help="ETS Tier-3 start epoch — used as reference (default: 46)")
    parser.add_argument("--min_t1t2",        type=int,   default=15,
    ...
    parser.add_argument("--windows",         type=int,   nargs="+",
                        default=[3, 5, 7, 10])
    parser.add_argument("--min_epochs_list", type=int,   nargs="+",
                        default=[5, 10, 15],
    ...
    n          = len(val_losses)
    print(f"Epochs  : {n}  |  milestones: {args.milestones}  |  ETS Tier-3 ref: ep {args.ets_t3}")
    print(f"Search  : {len(args.taus)} taus x {len(args.windows)} windows x "
    ...
    for tau in args.taus:
        for window in args.windows:
            for min_ep in args.min_epochs_list:
                changes  = simulate_lps(val_losses, tau=tau, window=window,
                                        min_epochs=min_ep, milestones=args.milestones)
    ...
    for s, tau, window, min_ep, t1t2, t2t3, t3_ep in results:
        ...
        print(f"{tau:>6.3f}  {window:>4}  {min_ep:>6}  "
    ...
    print(f"{'─'*60}")
    print(f"  {'Rank':>4}  {'tau':>6}  {'win':>4}  {'min_ep':>6}  "
          f"{'T2->T3':>8}  {'T3 eps':>7}  {'score':>7}")
    for rank, (s, tau, window, min_ep, t1t2, t2t3, t3_ep) in enumerate(results[:args.top], 1):
        t2t3_s = f"ep {t2t3}" if t2t3 else "never"
        t3ep_s = f"{t3_ep}"   if t2t3 else "—"
        print(f"  {rank:>4}  {tau:>6.3f}  {window:>4}  {min_ep:>6}  "
    ...
    if results[0][0] > -999:
        ...
        print(f"    --lps_tau {best[1]} --lps_window {best[2]} --lps_min_epochs {best[3]}")
