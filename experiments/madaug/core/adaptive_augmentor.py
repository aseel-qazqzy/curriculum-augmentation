...
# [madaug-adapt] device for tensors the official code moves with .cuda(); set by train_madaug.py
...
class MDAAug(nn.Module):
    ...
    def predict_aug_params(self, images, mode):
        ...
        if mode == 'exploit':
            ...
            self.h_model.train()
            T = 1.0
        a_params = self.h_model(self.gf_model.f(images.to(DEVICE)))
    ...
    def get_aug_valid_imgs(self, images, magnitudes):
        ...
        for i, image in enumerate(images):
            ...
            for k, ops_name in enumerate(self.ops_names):
                ...
                trans_image = stop_gradient(trans_image.to(DEVICE), magnitudes[i][k])
    ...
    def get_training_aug_images(self, images, magnitudes, weights):
        ...
        aug_imgs = torch.stack(trans_images, dim=0).to(DEVICE)
        return aug_imgs
