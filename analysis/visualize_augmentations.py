...
def visualize(dataset="cifar100", aug_type="curriculum", tiers=None, n=6, data_root="data"):
    ...
    original = torch.stack([pil_to_normed_tensor(tensor_to_pil(img), mean, std) for img in raw])
    ...
    gs = gridspec.GridSpec(
        ...
        left=0.18, right=0.98, top=0.93, bottom=0.02,
    )
    ...
    for r, (label, aug_batch) in enumerate(rows):
        for i in range(n):
            if has_aug:
                ...
                if r == 0:
                    ax_o.set_title(f"orig {i+1}", fontsize=6.5, pad=3,
                                   color="#888888")
                    ax_a.set_title(f"aug {i+1}", fontsize=6.5, pad=3,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--aug",     default="curriculum",
    ...
    parser.add_argument("--n",       type=int, default=6,
                        help="Number of image columns (default: 6)")
