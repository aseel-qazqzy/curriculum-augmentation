...
> Full 100-class table saved to: `results/perclass_accuracy.txt`

### Bottom-10 classes (hardest) — Static Mixing ranking

| Rank | Class | No Aug | Static | ETS | LPS | EGS v2 |
|:---:|:---|:---:|:---:|:---:|:---:|:---:|
| 1 | girl | 54% | 42% | 54% | 54% | 48% |
| 2 | otter | 45% | 44% | 59% | 52% | 70% |
| 3 | seal | 43% | 46% | 56% | 55% | 54% |
| 4 | mouse | 56% | 54% | 67% | 72% | 70% |
| 5 | boy | 49% | 56% | 54% | 55% | 63% |
| 6 | bear | 56% | 57% | 72% | 71% | 66% |
| 7 | squirrel | 67% | 58% | 82% | 78% | 82% |
| 8 | maple_tree | 63% | 58% | 65% | 67% | 66% |
| 9 | shrew | 53% | 59% | 67% | 64% | 65% |
| 10 | lizard | 57% | 59% | 72% | 68% | 73% |

> The hardest classes for Static Mixing split into two groups. People (girl, boy) and small similar-looking animals (otter, seal, mouse, shrew, squirrel, lizard, bear) suffer from high inter-class visual similarity. Static mixing applies CutMix/MixUp from epoch 1, which can merge visually ambiguous images and compound the confusion — girl drops to 42%, the lowest of all 100 classes. Curriculum methods (ETS/LPS) recover girl to 54% (+12pp) and squirrel to 78–82% (+20–24pp) by delaying mixing until the model has built stable representations.

### Top-10 classes (easiest)
...
### Class-level gains: Static → ETS / LPS / EGS v2

**Static → ETS — top 10 gains**
...
**Static → EGS v2 — top 10 regressions**
...
> Consistent pattern across all three curriculum methods: the largest gains are on fine-grained animals that look similar to each other — squirrel, mouse, bear, lizard, otter, forest. These classes share body shapes or background textures and are hardest under static mixing which exposes the model to their mixed versions from the start. Progressive curriculum methods build initial representations on clean geometric transformations before introducing high-distortion mixing, which helps the model first learn discriminative features before seeing ambiguous mixed images.
>
> Consistent regressions appear on high-baseline classes where static mixing already achieves near-ceiling accuracy (road 98%, chair 95%, mushroom 87%) and on certain texture classes (bowl, oak_tree, butterfly) where curriculum ordering appears to disrupt features that static mixing builds well. The bowl/oak_tree/butterfly regressions are consistent across ETS, LPS, and EGS v2, suggesting these are classes that benefit more from early high-distortion exposure than from the gradual progression.
>
> EGS v2 shows the largest single-class gain of any method: otter +26pp (44%→70%). EGS's per-sample scheduling promotes each sample independently, and otter samples appear to be among the earliest to reach Tier 3 — giving them the most mixing exposure of any curriculum method. This contrasts with EGS's overall lower mean accuracy, suggesting the per-sample approach redistributes rather than uniformly improves accuracy.
