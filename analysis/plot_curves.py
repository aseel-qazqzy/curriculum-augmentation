...
def load_history(name, checkpoint_dir=CHECKPOINT_DIR):
    ...
    if len(parts) == 1:
        path, h = parts[0]
        print(f"  {os.path.basename(path):<55}  {len(h['val_acc']):>3} epochs  "
...
def lr_vlines(ax, n, color="#888888", alpha=0.35):
    ...
    for i, m in enumerate(ms):
        if m < xhi:
            ax.axvline(m, color=color, lw=0.9, ls=":", alpha=alpha,
...
def tier_vlines(ax, color="#009E73", alpha=0.60):
    ...
    for i, ep_t in enumerate(TIER_EPOCHS):
        ax.axvline(ep_t, color=color, lw=1.1, ls="--", alpha=alpha,
                   label=f"↑ {labels[i]}" if i == 0 else "_")
        ax.text(ep_t + 0.8, ax.get_ylim()[1] * 0.97,
                labels[i], fontsize=7, color=color, va="top", alpha=0.85)
...
def fig1_val_comparison(runs, title="", fname="fig1_val_comparison.png"):
    ...
    for label, (h, color) in runs.items():
        ...
        ep_b, val_b = best(h)
        axes[1].scatter([ep_b], [val_b], color=color, s=45, zorder=6,
    ...
    fig.legend(handles + [lr_p], labs + ["MultiStepLR decay"],
               ...
               bbox_to_anchor=(0.5, -0.05), frameon=True, fontsize=8)
...
def fig3_generalization(runs, fname="fig3_generalization.png"):
    ...
    for i, (label, h, color) in enumerate(valid):
        ...
        axes[i].fill_between(x, va, ta, alpha=0.12, color=color,
                             label=f"Gap = {float(ta[-1]-va[-1]):.1f}%")

        ep_b, val_b = best(h)
        axes[i].scatter([ep_b], [val_b], color=color, s=55, zorder=6,
...
def fig4_lr_schedule(n_epochs=100, fname="fig4_lr_schedule.png"):
    ...
    ax.plot(ep_x, lr_ms, color=PALETTE["static"], lw=2.2,
    ...
    for j, ep_t in enumerate(TIER_EPOCHS):
        ax.axvline(ep_t, color=tier_colors[j], lw=1.2, ls="--", alpha=0.75,
                   label=f"ep {ep_t}: {tier_labels[j]}")
        ax.text(ep_t + 0.5, lr_ms[0] * 1.12, f"ep {ep_t}\n{tier_labels[j]}",
                fontsize=7, color=tier_colors[j], va="bottom")
...
def fig5_summary(runs, fname="fig5_summary.png"):
    ...
    bars  = ax0.bar(x_pos, best_v, color=colors, edgecolor="black",
    ...
    bars2 = ax1.bar(x_pos, gaps, color=colors, edgecolor="black",
                    linewidth=0.6, width=0.55, zorder=3)
    ...
    for label, h, color in valid:
        ...
        ls   = "--" if "CL" in label or "Tier" in label else "-"
        lw   = 2.2 if "CL" in label or "Tier" in label else 1.8
        ax3.plot(x, g, color=color, lw=lw, ls=ls,
                 label=label.split("(")[0].strip())
...
def fig6_cl_with_tiers(runs, fname="fig6_cl_tier_transitions.png"):
    ...
    for key, color, ls, lw in [
        ...
        ep_b, val_b = best(h)
        ax.scatter([ep_b], [val_b], color=color, s=55, zorder=6,
                   edgecolors="white", lw=0.8)
        ax.annotate(f"  {val_b:.2f}%", xy=(ep_b, val_b),
                    fontsize=7.5, color=color, fontweight="bold")
        all_acc.extend(va)

    # Tier transition vertical markers
    tier_colors = ["#009E73", "#E69F00"]
    tier_names  = ["Tier 2 unlocked\n(+jitter, rotation)", "Tier 3 unlocked\n(+cutout, grayscale)"]
    ...
    for ep_t, tcolor, tname in zip(TIER_EPOCHS, tier_colors, tier_names):
        ...
        ax.text(ep_t + 0.8, yhi - (yhi - ylo) * 0.04,
                tname, fontsize=7, color=tcolor, va="top")

    # Shaded regions for each tier
    tier_boundaries = [1] + TIER_EPOCHS + [100]
    tier_bg_colors  = ["#F0F8FF", "#F0FFF0", "#FFFFF0"]
    tier_labels_bg  = ["Tier 1\n(flip, crop)", "Tier 2\n(+jitter, rot.)", "Tier 3\n(+cutout)"]
    for j in range(len(tier_bg_colors)):
        ax.axvspan(tier_boundaries[j], tier_boundaries[j+1],
                   alpha=0.18, color=tier_bg_colors[j], zorder=0)
        mid = (tier_boundaries[j] + tier_boundaries[j+1]) / 2
        ax.text(mid, ylo + (yhi - ylo) * 0.04,
                tier_labels_bg[j], ha="center", fontsize=6.5,
...
def fig8_top5(runs, fname="fig8_top5_comparison.png"):
    ...
    """
    valid = [(k, h, c) for k, (h, c) in runs.items()
    ...
    bars1 = ax.bar(x_pos - width/2, top1v, width, label="Top-1",
                   color=colors, edgecolor="black", linewidth=0.6, zorder=3)
    bars2 = ax.bar(x_pos + width/2, top5v, width, label="Top-5",
                   color=colors, edgecolor="black", linewidth=0.6,
                   alpha=0.50, hatch="///", zorder=3)
...
def print_analysis(runs):
    ...
    print(f"\n{bar}")
    print("THOROUGH ANALYSIS — CIFAR-100  ·  ResNet-50  ·  SGD  ·  MultiStepLR [33,66,83]")
    print(bar)
    ...
    no_aug = next((r for r in rows if "no" in r["label"].lower()
                   and "aug" in r["label"].lower()), None)
    ...
    if no_aug:
        print(f"  {idx}. No-augmentation overfitting\n"
...
def mode_baselines(ckpt):
    ...
    runs = build_runs([
        ("No Augmentation (floor)",         "resnet50_no_aug_cifar100_v2_cifar100",     PALETTE["no_aug"]),
        ("Static Aug (all ops, epoch 1)",   "resnet50_static_aug_cifar100_v2_cifar100", PALETTE["static"]),
    ], ckpt)
    print("\n  Generating figures + analysis...")
    fig1_val_comparison(runs, title="Baseline Comparison — CIFAR-100 · ResNet-50",
...
def mode_all(ckpt):
    ...
    runs = build_runs([
        ("No Augmentation (floor)",          "resnet50_no_aug_cifar100_v2_cifar100",     PALETTE["no_aug"]),
        ("Static Aug (all ops, epoch 1)",    "resnet50_static_aug_cifar100_v2_cifar100", PALETTE["static"]),
        ("Tiered CL (3-tier progressive) *", "resnet50_tiered_curriculum_cifar100",      PALETTE["cl"]),
    ], ckpt)
    print("\n  Generating figures + analysis...")
    fig1_val_comparison(runs, title="All Methods — CIFAR-100 · ResNet-50",
