...
def parse_log(path: Path) -> dict:
    ...
    full_train  = mode_match and "full-train" in mode_match.group(1) if mode_match else False
...
def report_dev(runs: list[dict]) -> None:
    ...
    fields = ["exp_name", "best_val_acc", "best_epoch", "val_test_gap",
              "test_top1", "test_top5", "last_train_loss", "last_train_acc"]
...
def report_curves(runs: list[dict]) -> None:
    ...
    fields = ["experiment", "epoch", "train_loss", "train_acc",
    ...
    print(f"Curves CSV saved → {out}  ({sum(len(r['epochs']) for r in runs)} epoch rows)")
...
def main() -> None:
    ...
    parser.add_argument("--report_mode", choices=list(REPORT_MODES), default="dev",
